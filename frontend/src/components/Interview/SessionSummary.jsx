...
function QuestionReview({ answers }) {
  ...
  const scoreColor = (s) => s >= 8 ? '#4ade80' : s >= 6 ? '#fbbf24' : s >= 4 ? '#f97316' : '#f87171';
    ...
    <div style={{ marginBottom: '24px' }}>
      <button
        ...
        style={{
          ...
          borderRadius: '12px', padding: '14px 18px', color: '#f1f5f9',
            ...
            return (
              <div key={i} style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: '12px', overflow: 'hidden' }}>
                ...
                >
                  ...
                  <span style={{ flex: 1, color: '#e2e8f0', fontSize: '13.5px', fontWeight: 600, lineHeight: 1.45 }}>
                    {a.question}
                  </span>
                  <span style={{ flexShrink: 0, color: scoreColor(a.score || 0), fontSize: '15px', fontWeight: 800 }}>
                    {a.score ?? '–'}<span style={{ color: '#334155', fontSize: '11px' }}>/10</span>
                  </span>
                </button>
                {isOpen && (
                  <div className="fade-in" style={{ padding: '0 16px 16px', borderTop: '1px solid rgba(255,255,255,0.05)' }}>
                    ...
                    <p style={{ color: '#94a3b8', fontSize: '13px', lineHeight: 1.6, whiteSpace: 'pre-wrap' }}>{a.answer}</p>
                    {ev.feedback && (<>
                      ...
                      <p style={{ color: '#94a3b8', fontSize: '13px', lineHeight: 1.6 }}>{ev.feedback}</p>
                    </>)}
                    {ev.model_answer && (<>
                      <p style={{ color: '#818cf8', fontSize: '11px', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.08em', margin: '14px 0 6px' }}>Model answer</p>
                      <p style={{ color: '#94a3b8', fontSize: '13px', lineHeight: 1.6 }}>{ev.model_answer}</p>
                    </>)}
                  </div>
                )}
              </div>
